...
import { hop, addr, bytes, b58, fromB58, signReach, assertAbi } from './ffi.mjs'
...
export class HopEndpoint extends EventEmitter {
  ...
  cluster(secretOrPassphrase) {
    if (typeof secretOrPassphrase === 'string') {
      ...
      hop.cluster_join_passphrase(this.#node, buf, buf.length)
    } else {
  ...
  get clusterMembers() {
    return hop.cluster_members(this.#node)
  }
  ...
  // the hold (the default). Also settable via the `quorum` constructor option. Returns this.
  clusterQuorum(min) {
    hop.cluster_set_quorum(this.#node, min >>> 0)
    return this
  }
  ...
  signReach(endpoint, ttlSecs = 3600) {
    return signReach(this.#node, endpoint, ttlSecs)
  }
  ...
  #makeReply(req) {
    ...
    return (status, body = Buffer.alloc(0)) => {
      ...
      const b = toBuf(body)
      return hop.send_service_response(this.#node, req._raw, req._rid, status, b, b.length)
    }
  }
