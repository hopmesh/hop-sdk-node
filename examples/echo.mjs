...
server.on('acme/orders', async (req, reply) => {
  ...
  reply.send(200, { ok: true, id: 42, item: order.item }) // uint16 status, JSON body
})
...
const res = await client.request(server.addressBytes, 'acme/orders', 'create', { item: 'widget' })
...
server.close()
client.close()
console.log(passed ? '\nPASS: hop.on(service, handler) + reply.send(status, body) over real hop-core.' : '\nFAIL')
